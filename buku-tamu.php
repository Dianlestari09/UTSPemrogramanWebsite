...
$komen_qry = $mysqli->query("SELECT nama, komentar FROM buku_tamu WHERE status = 'ya' ORDER BY tanggal DESC LIMIT ".$limit) or die($mysqli->error);

$sql_rec = "SELECT nama, komentar FROM buku_tamu WHERE status = 'ya'";
								...
								<div class="col-sm-12">
									...
									<?php } else { ?>
										...
										<div class="buku-tamu-item">
											<h4 class="page-header">Dari: <?php echo $komen['nama'];?></h4>
											<p class="text-muted"><?php echo $komen['komentar'];?></p>
										</div>
										<?php } ?>
									<?php } ?>
									</div>
